...
OUT_DIR  = REPO / "renders" / "sweep_audition"
...
PROMPTS = {
    "techno":    "techno loop, snappy bassline, powerful drum, 909 hihat, korg, roland, upbeat",
    "guitar":    "acoustic steel-string guitar finger picking, folk music mellow",
    "oboe":      "solo oboe, bittersweet, dorian scale, folk music, cinematic",
    "breakbeat": "breakbeat drum loop, funky style",
}
DEFAULT_PROMPT_TAG = "techno"
...
VARIANTS = [
    # §21 production ship heads
    {"name": "ship_sfn_bass",      "feature": "rms_energy_bass",
     "ckpt": REPO / "latch_weights" / "test" / "latch_rms_energy_bass_ship_rms_energy_bass_sfn_s1_best.pt"},
    {"name": "ship_ff_bass",       "feature": "rms_energy_bass",
     "ckpt": REPO / "latch_weights" / "test" / "latch_rms_energy_bass_ship_rms_energy_bass_fullfusion_s1_best.pt"},
    {"name": "ship_sfn_flatness",  "feature": "spectral_flatness",
     "ckpt": REPO / "latch_weights" / "test" / "latch_spectral_flatness_ship_spectral_flatness_sfn_s1_best.pt"},
    {"name": "ship_sfn_flux",      "feature": "spectral_flux",
     "ckpt": REPO / "latch_weights" / "test" / "latch_spectral_flux_ship_spectral_flux_sfn_s1_best.pt"},
    # Our most recent Fusion-trained whole-track head (per-stem drums onset)
    {"name": "wt_fusion_drums_best",  "feature": "onset_envelope_drums",
     "ckpt": REPO / "latch_weights" / "latch_onset_envelope_drums_wt_fusion_best.pt"},
    {"name": "wt_fusion_drums_last",  "feature": "onset_envelope_drums",
     "ckpt": REPO / "latch_weights" / "latch_onset_envelope_drums_wt_fusion_last.pt"},
]

# Snapshot trajectories (ep10/15/20/25/30) — user asked for "ep15 if available".
...
TARGET_VALUES = {
    ...
    "onset_envelope_drums": 0.8,      # mid-range onset density
}
...
if __name__ == "__main__":
    ...
    legacy_variants = set()  # multi-hparam legacy disabled; all variants get canonical-hparam multi-prompt
