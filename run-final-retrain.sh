...
#
# torch.compile is ON: ~+45% it/s, half the energy per item, val_median unchanged
# (verified on onsets_activations 2026-05-28 — see LATCH_RESULTS.txt §16).
# First head pays a one-time ~60s Triton autotune cost; subsequent heads (same shape)
# reuse the cached graph (TORCHINDUCTOR_FX_GRAPH_CACHE=1 in rocm_env.yaml).
#
