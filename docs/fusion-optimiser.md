...
| **MONA** | EMA of gradient differences injected into momentum as a cheap proxy for second-order curvature; deflects updates away from sharp minima. | [arXiv:2605.26842](https://arxiv.org/abs/2605.26842) — *MONA: Muon Optimizer with Nesterov Acceleration for Scalable Language Model Training* (Jiacheng Li et al., May 2026). Validated on 1B–68B-param MoE pretraining. Our per-component bake-off at LatCH's 5M-param scale shows MONA *worsens* pure Muon (+5.8 % vs +2.7 %) — the curvature deflection appears scale-dependent and isn't a free win at our size. |
...
All seven inspirations are now confirmed published. MONA's recipe is real, just not load-bearing at LatCH's 5 M-param scale; it's not part of the SF-NorMuon shipping target, so the composition we ended up wanting to ship is entirely backed by recipes that work at our scale.
...
> **Important**: `--hot-dtype fp16` looked promising in the microbench (2.65× speedup) but **diverges to NaN after ~2 steps** in practice. The NS5 quintic's iterated `X (XᵀX)²` overflows fp16's 65 504 max during the first optimiser step on LatCH's 1024×256 mats. bf16 has fp32-equivalent exponent range (8-bit), runs stably, costs 1 % on val_point_mae, and is ~1.65× faster than fp32. We tested whether re-tuning the bf16 kernels in hipBLASLt could push that further — answer is no, the bf16 kernels are already near-optimal for our 256-grid shapes; 26 it/s is the ceiling. **We also tested `--hot-dtype fp16_safe`** (fp16 matmuls + fp32 polynomial accumulation + per-tensor rescale-restore inside NS5): the rescale-restore overhead (~6 max-reductions per matmul × 480 matmuls/step) DOMINATES, making fp16_safe 0.5× bf16. **bf16 is the plateau on this hardware/software stack.** A custom Triton NS5 kernel with rescale fused into the matmul epilogue could in principle reclaim the predicted ~1.5× — that's ~200 LoC of real engineering, not a hot-fix. See LATCH_RESULTS.txt §20 Parts D + E.
