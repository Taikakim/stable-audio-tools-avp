...
- **NorMuon** — *Making Muon more efficient and scalable*, [arXiv:2510.05491](https://arxiv.org/abs/2510.05491) (October 2025). Introduces the per-neuron row normalisation we use in the `normuon` component. *(verified after the bake-off — the spec's original "provenance unverified" was wrong here.)*
...
- **SF-NorMuon** — *Anytime Training with Schedule-Free Spectral Optimization*, [arXiv:2605.23061](https://arxiv.org/abs/2605.23061) (May 2026). Combines Schedule-Free averaging with NorMuon spectral updates, with weight decay on the fast iterate for long-horizon stability. This is the exact recipe behind the `--components ns5,normuon,sf` configuration that the bake-off identified as the simplest standalone winner. *(verified after the bake-off — the spec's original "provenance unverified" was wrong here.)*
- **MONA** — [arXiv:2605.26842](https://arxiv.org/abs/2605.26842), *MONA: Muon Optimizer with Nesterov Acceleration for Scalable Language Model Training* (Jiacheng Li et al., May 2026). EMA-of-gradient-differences curvature term injected into Muon's momentum to help escape sharp minima while preserving Muon's spectral-norm bound. Validated on 1B–68B-param MoE pretraining with SOTA on general/math/code downstream tasks. *(My earlier "provenance suspect" call was wrong — the user found this paper and it's real.) Per-component bake-off at LatCH's 5M-param scale shows MONA + Muon WORSENS pure Muon (+5.8 % vs +2.7 %), suggesting the curvature deflection is scale-dependent and isn't load-bearing at our model size. Not included in the SF-NorMuon shipping target.*
- **ScheduleFree+** — [arXiv:2605.19095](https://arxiv.org/abs/2605.19095), *Scaling Learning-Rate-Free & Schedule-Free Learning to Large Language Models* (Aaron Defazio, May 2026). Polyak step size with model averaging and checkpoint merging; +31 % over Warmup-Stable-Decay at 1000 tokens/param on LLMs. *(verified post-hoc.)*
- **KL-Shampoo** — [arXiv:2509.03378](https://arxiv.org/abs/2509.03378), *Understanding and Improving Shampoo and SOAP via Kullback-Leibler Minimization* (Wu Lin, Scott C. Lowe, Felix Dangel, Runa Eschenhagen, Zikun Xu, Roger B. Grosse, September 2025). Reframes second-moment estimation as KL divergence minimisation; matches Shampoo/SOAP without Adam's memory grafting. *(verified post-hoc.) Per-component bake-off shows KL-Shampoo standalone diverges to NaN without NS5's spectral-norm clipping — consistent with the classical Shampoo instability the paper addresses through grafting; our composition with NS5 turns out to be the equivalent stability mechanism.*
...
- `LATCH_RESULTS.txt` §19, §20 — Phase 1 bake-off, Phase 2 seed-variance test, microbench profile, shrink experiment, per-component ablation. Empirical validation of this spec.
- `docs/fusion-optimiser.md` — the empirical report; consolidated findings in one place with the production recommendation (SF-NorMuon at d256/dp4).
