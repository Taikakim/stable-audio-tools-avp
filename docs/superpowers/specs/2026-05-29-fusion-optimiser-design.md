...
- **MONA** — claimed as "Muon Optimizer with Nesterov Acceleration" (May 2026) in `docs/ideas/optimiser_ideas`. Adds an EMA-of-gradient-differences curvature term to Muon's momentum. *(**Provenance suspect.** The only "MONA" paper located is a DeepMind AI-safety paper — "Myopic Optimization with Non-myopic Approval", Farquhar et al. Jan 2025 — that's unrelated to optimisers. The "Muon + Nesterov" recipe in our ideas doc may be a name collision or fabrication. Per-component bake-off below shows MONA + Muon WORSENS pure Muon (+5.8 % vs +2.7 %), consistent with the math being speculative. Treat as an experimental component, not a validated recipe.)*
